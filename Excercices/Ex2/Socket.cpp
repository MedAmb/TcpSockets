...
Socket::Socket(int domain, int type, int protocol, int port, bool reuse, socketType sockettype, bool waitForServer = false):
m_domain(domain),
m_type(type),
m_protocol(protocol),
...
int Socket::m_send(const std::string& message)
{
    ...
    {
        s_return = send(m_socketId, message.c_str(), 16, 0);
    }
